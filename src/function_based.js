const SECTION_MODULE = (main) => {
    const createSection = (title, content, index) => {
        ...
        section.innerHTML = `
            <h3 class="heading">
              <button aria-expanded="false" tabindex="${index === 0 ? 0 : -1}" data-index="${index}">${title === 'user' ? content : 'Response'}</button>
            </h3>
            <div class="content" role="region">
                <div><p>${content.replaceAll("\n","</br>")}</p></div>
            </div>
        `;
...
const ACCORDION_MODULE = (main) => {
    const getData = async () => {
        try {
            const data = await fetch("../java.json");
            const posts = await data.json();
            posts.messages.forEach((post, index) => {
                SECTION_MODULE(main).createSection(post.role, post.content, index);
            });
        } catch (error) {
            console.log(error);
        }
